...
ComputationProver(int _num_vars, int _num_cons, int _size_input, int _size_output, 
                  mpz_t _prime, const char *_shared_bstore_file_name, string inputFilename, bool only_setup, bool need_db, vector<uint32_t> * inputList):
    ...
    shared_bstore_file_name(string(_shared_bstore_file_name))
{
    ...
    if(inputList!=NULL)
    {
      ...
      for (std::vector<uint32_t>::iterator it = inputList->begin(); it != inputList->end(); it++)
      {
        ...
        assert(counter < size_input);
        mpq_set_ui(input_q[counter++], *it, 1);
      }
      //_mm_free(inputList); // we use these only once.
    }
    else
    {
...
static int try_next_token(char *fp, char* token, uint32_t &offset) {
  int sz;
  auto ret = sscanf(fp+offset, "%s%n", token, &sz);
  offset += sz;
  if(offset >= pws_length) return EOF;
  uint32_t percentage = offset / (pws_length/100);
  if(percentage > pws_progress)
  {
    std::cout << "prover computation: " << percentage << "%" << std::endl;
    pws_progress = percentage;
  }
  return ret;
}
...
static void next_token_or_error(char* fp, char* token, uint32_t &offset) {
  int sz;
  int ret = sscanf(fp+offset, "%s%n", token, &sz);
  offset += sz;
  if(offset >= pws_length) return ;
  uint32_t percentage = offset / (pws_length/100);
  if(percentage > pws_progress)
  {
    std::cout << "prover computation: " << percentage << "%" << std::endl;
...
static void expect_next_token(char* fp, const char* expected, const char* error, uint32_t &offset) {
  char buf[BUFLEN];
  next_token_or_error(fp, buf, offset);
...
void ComputationProver::init_block_store() {
  ...
  std::cout << "bstore_file_path_priv_str " << bstore_file_path_priv_str << std::endl;
...
void ComputationProver::compute_poly(char* pws, int tempNum, uint32_t & offset) {
  ...
  while(try_next_token(pws, tok, offset) != EOF) {
    ...
    if (strcmp(tok, "(") == 0) {
      //Recurse
      compute_poly(pws, tempNum + 2, offset);
...
void ComputationProver::compute_split_unsignedint(char* pws_file, uint32_t &offset) {
  mpq_t* in = NULL;
  char cmds[BUFLEN];

  //cout << *cmds << endl;
  next_token_or_error(pws_file, cmds, offset);
  ...
  expect_next_token(pws_file, "into", "Invalid SI", offset);
  next_token_or_error(pws_file, cmds, offset);
  int length = atoi(cmds);
  expect_next_token(pws_file, "bits", "Invalid SI", offset);
  expect_next_token(pws_file, "at", "Invalid SI", offset);
  ////cout << *cmds << endl;
  next_token_or_error(pws_file, cmds, offset);
...
void ComputationProver::compute_split_int_le(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  next_token_or_error(pws_file, cmds, offset);
  bool isSigned = cmds[0] != 'u';
  expect_next_token(pws_file, "bits", "Invalid SIL", offset);
  next_token_or_error(pws_file, cmds, offset);
  int N = atoi(cmds);
  expect_next_token(pws_file, "X", "Invalid SIL", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& in = voc(cmds, temp_q);
  expect_next_token(pws_file, "Y0", "Invalid SIL", offset);
  next_token_or_error(pws_file, cmds, offset);
  if (cmds[0] != 'V'){
...
void ComputationProver::compute_less_than_int(char* pws, uint32_t &offset) {
  char cmds[BUFLEN];

  expect_next_token(pws, "N_0", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  int N_start = atoi(cmds + 1);
  expect_next_token(pws, "N", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  int N = atoi(cmds);
  expect_next_token(pws, "Mlt", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  mpq_t& Mlt = voc(cmds, temp_q);
  expect_next_token(pws, "Meq", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  mpq_t& Meq = voc(cmds, temp_q);
  expect_next_token(pws, "Mgt", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  mpq_t& Mgt = voc(cmds, temp_q);

  expect_next_token(pws, "X1", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  mpq_t& X1 = voc(cmds, temp_q);
  expect_next_token(pws, "X2", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  mpq_t& X2 = voc(cmds, temp_q);
  expect_next_token(pws, "Y", "Invalid <I", offset);
  next_token_or_error(pws, cmds, offset);
  mpq_t& Y = voc(cmds, temp_q);
...
void ComputationProver::compute_less_than_float(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];
  expect_next_token(pws_file, "N_0", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  int N_start = atoi(cmds + 1);

  expect_next_token(pws_file, "Na", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  int Na = atoi(cmds);

  expect_next_token(pws_file, "N", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& N = voc(cmds, temp_q);

  expect_next_token(pws_file, "D_0", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  int D_start = atoi(cmds + 1);

  expect_next_token(pws_file, "Nb", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  int Nb = atoi(cmds);

  expect_next_token(pws_file, "D", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  ...
  expect_next_token(pws_file, "Mlt", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& Mlt = voc(cmds, temp_q);
  expect_next_token(pws_file, "Meq", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& Meq = voc(cmds, temp_q);
  expect_next_token(pws_file, "Mgt", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& Mgt = voc(cmds, temp_q);

  expect_next_token(pws_file, "X1", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& X1 = voc(cmds, temp_q);
  expect_next_token(pws_file, "X2", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& X2 = voc(cmds, temp_q);
  expect_next_token(pws_file, "Y", "Invalid <I", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& Y = voc(cmds, temp_q);
...
void ComputationProver::compute_db_get_bits(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];
  next_token_or_error(pws_file, cmds, offset);
...
void ComputationProver::compute_db_put_bits(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& index = voc(cmds, temp_q);
  uint32_t idx = mpz_get_ui(mpq_numref(index));
...
void ComputationProver::compute_exo_compute(char *pws_file, uint32_t &offset) {
    // read in the EXO_COMPUTE line
    char cmds[BUFLEN];

    // grab exoid
    expect_next_token(pws_file, "EXOID", "Invalid EXO_COMPUTE", offset);
    next_token_or_error(pws_file,cmds, offset);
    int exoId = atoi(cmds);

    expect_next_token(pws_file, "INPUTS", "Invalid EXO_COMPUTE", offset);
    expect_next_token(pws_file, "[", "Invalid EXO_COMPUTE", offset);
    std::vector< std::vector<std::string> > inVarsStr;
    // get the input args
    getLL(inVarsStr, pws_file, cmds, offset);

    expect_next_token(pws_file, "OUTPUTS", "Invalid EXO_COMPUTE", offset);
    expect_next_token(pws_file, "[", "Invalid EXO_COMPUTE", offset);
    std::vector<std::string> outVarsStr;
    // get the output args
    getL(outVarsStr, pws_file, cmds, offset);
...
void ComputationProver::compute_ext_gadget(char *pws_file, uint32_t &offset) {
    // read in the EXO_COMPUTE line
    char cmds[BUFLEN];

    // grab exoid
    expect_next_token(pws_file, "GADGETID", "Invalid EXT_GADGET", offset);
    next_token_or_error(pws_file,cmds, offset);
    int gadgetId = atoi(cmds);

    expect_next_token(pws_file, "INPUTS", "Invalid EXT_GADGET", offset);
    expect_next_token(pws_file, "[", "Invalid EXT_GADGET", offset);
    std::vector<std::string> inVarsStr;
    // get the input args
    getL(inVarsStr, pws_file, cmds, offset);

    expect_next_token(pws_file, "OUTPUTS", "Invalid EXT_GADGET", offset);
    expect_next_token(pws_file, "[", "Invalid EXT_GADGET", offset);
    std::vector<std::string> outVarsStr;
    // get the output args
    getL(outVarsStr, pws_file, cmds, offset);

    expect_next_token(pws_file, "INTERMEDIATE", "Invalid EXT_GADGET", offset);
    expect_next_token(pws_file, "[", "Invalid EXT_GADGET", offset);
    std::vector<std::string> intermediateVarsStr;
    // get the output args
    getL(intermediateVarsStr, pws_file, cmds, offset);
...
void ComputationProver::getLL(std::vector< std::vector<std::string> > &inLL, char *pws_file, char *buf, uint32_t &offset) {
    // pull until we get ] ]
    while(1) {
        // if we have ] then we're done
        next_token_or_error(pws_file, buf, offset);
        ...
        getL(tmpList,pws_file,buf, offset);
...
void ComputationProver::getL(std::vector<std::string> &inL, char *pws_file, char *buf, uint32_t &offset) {
    while(1) {
        next_token_or_error(pws_file, buf, offset);
        if(strcmp(buf,"]") == 0) { break; }
...
void ComputationProver::compute_fast_ramget(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  mpq_t addr;
  mpq_init(addr);

  expect_next_token(pws_file, "ADDR", "Invalid RAMGET_FAST", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_set(addr, voc(cmds, temp_q));

  expect_next_token(pws_file, "VALUE", "Invalid RAMGET_FAST", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& value = voc(cmds, temp_q);
...
void ComputationProver::compute_fast_ramput(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  mpq_t addr, value, condition;
  mpq_inits(addr, value, condition, NULL);

  expect_next_token(pws_file, "ADDR", "Invalid RAMPUT_FAST", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_set(addr, voc(cmds, temp_q));

  expect_next_token(pws_file, "VALUE", "Invalid RAMPUT_FAST", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_set(value, voc(cmds, temp_q));

  expect_next_token(pws_file, "CONDITION", "Invalid RAMPUT_FAST", offset);
  next_token_or_error(pws_file, cmds, offset);
  mpq_set(condition, voc(cmds, temp_q));
  next_token_or_error(pws_file, cmds, offset);
...
void ComputationProver::compute_db_get_sibling_hash(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];
  next_token_or_error(pws_file, cmds, offset);
  mpq_t& index = voc(cmds, temp_q);
  uint32_t idx = mpz_get_ui(mpq_numref(index));
...
void ComputationProver::parse_hash(char* pws_file, HashBlockStore::Key& outKey, int numHashBits, uint32_t &offset) {
  char cmds[BUFLEN];
...
void ComputationProver::compute_get_block_by_hash(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  HashBlockStore::Key key;
  parse_hash(pws_file, key, _ram->getNumHashBits(), offset);
  ...
  expect_next_token(pws_file, "NUM_Y", "compute_get_block_by_hash: expected NUM_Y", offset);

  next_token_or_error(pws_file, cmds, offset);
  mpq_t& nb = voc(cmds, temp_q);
  uint32_t numBits = mpz_get_ui(mpq_numref(nb));

  expect_next_token(pws_file, "Y", "compute_get_block_by_hash: expected Y", offset);
...
void ComputationProver::compute_put_block_by_hash(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];
  ...
  expect_next_token(pws_file, "NUM_X", "compute_put_block_by_hash: expected NUM_X", offset);

  next_token_or_error(pws_file, cmds, offset);
  mpq_t& nb = voc(cmds, temp_q);
  uint32_t numBits = mpz_get_ui(mpq_numref(nb));

  expect_next_token(pws_file, "X", "compute_put_block_by_hash: expected X", offset);
...
void ComputationProver::compute_free_block_by_hash(char* pws_file, uint32_t &offset) {
  HashBlockStore::Key key;
  parse_hash(pws_file, key, _ram->getNumHashBits(), offset);
  _blockStore->free(key);
}
...
void ComputationProver::compute_printf(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];
  ...
  expect_next_token(pws_file, "X", "Format error in printf", offset);
...
void ComputationProver::compute_genericget(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  expect_next_token(pws_file, "COMMITMENT", "For now, only COMMITMENT is allowed in genericGET", offset);

  expect_next_token(pws_file, "NUM_HASH_BITS", "Format error in genericget 1", offset);

  next_token_or_error(pws_file, cmds, offset);

  expect_next_token(pws_file, "HASH_IN", "Format error in genericget 2", offset);

  HashBlockStore::Key key;
  parse_hash(pws_file, key, NUM_COMMITMENT_BITS, offset);
  ...
  expect_next_token(pws_file, "NUM_Y", "compute_genericget: expected NUM_Y", offset);

  next_token_or_error(pws_file, cmds, offset);
  mpq_t& nb = voc(cmds, temp_q);
  uint32_t numBits = mpz_get_ui(mpq_numref(nb));

  expect_next_token(pws_file, "Y", "compute_genericget: expected Y", offset);
...
void ComputationProver::compute_matrix_vec_mul(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  expect_next_token(pws_file, "NUM_ROWS", "Invalid MATRIX_VEC_MUL, NUM_ROWS exected.", offset);
  next_token_or_error(pws_file, cmds, offset);
  int number_of_rows = atoi(cmds);

  expect_next_token(pws_file, "NUM_COLUMNS", "Invalid MATRIX_VEC_MUL, NUM_COLUMNS expected.", offset);
  next_token_or_error(pws_file, cmds, offset);
  int number_of_columns = atoi(cmds);

  expect_next_token(pws_file, "ACTUAL_NUM_COLUMNS", "Invalid MATRIX_VEC_MUL, ACTUAL_NUM_COLUMNS expected.", offset);
  next_token_or_error(pws_file, cmds, offset);
  int actual_number_of_columns = atoi(cmds);

  // read input bits from F1
  expect_next_token(pws_file, "IN_VEC", "Invalid MATRIX_VEC_MUL, IN_VEC expected.", offset);
  ...
  expect_next_token(pws_file, "OUT_VEC", "Invalid MATRIX_VEC_MUL, IN_VEC expected.", offset);
...
void ComputationProver::compute_waksman_network(char* pws_file, uint32_t &offset) {
  char cmds[BUFLEN];

  expect_next_token(pws_file, "WIDTH", "Invalid WAKSMAN_NETWORK, WIDTH expected.", offset);
  next_token_or_error(pws_file, cmds, offset);
  int width = atoi(cmds);
  
  expect_next_token(pws_file, "INPUT", "WAKSMAN_NETWORK, INPUT expected.", offset);
  int num_switches = 0;
  for (int i = 1; i <= width; i++) {
    num_switches += ceil(log2(i));
   }
  ...
  expect_next_token(pws_file, "INTERMEDIATE", "Invalid WAKSMAN_NETWORK, INTERMEDIATE expected.", offset);
  next_token_or_error(pws_file, cmds, offset);
  if (strcmp(cmds, "NULL") != 0) {
    int intermediate_offset = atoi(cmds+1);
  ...
  expect_next_token(pws_file, "OUTPUT", "Invalid WAKSMAN_NETWORK, OUTPUT expected.", offset);
  next_token_or_error(pws_file, cmds, offset);
  int output_offset = atoi(cmds+1);
  ...
  expect_next_token(pws_file, "SWITCH", "Invalid BENES_NETWORK, SWITCH expected.", offset);
  next_token_or_error(pws_file, cmds, offset);
...
void ComputationProver::compute_from_pws_str(char* pws) {
  if (pws == NULL) {
  ...
  while(try_next_token(pws, tok, offset) != EOF) {
    if (strcmp(tok, "P") == 0) {
      next_token_or_error(pws, cmds, offset);
      mpq_t& Y = voc(cmds, temp_q);
      expect_next_token(pws, "=", "Invalid POLY", offset);
      compute_poly(pws, 0, offset);
      mpq_set(Y, temp_qs[0]);
    } else if (strcmp(tok, "<I") == 0) {
      compute_less_than_int(pws, offset);
    } else if (strcmp(tok, "<F") == 0) {
      compute_less_than_float(pws, offset);
    } else if (strcmp(tok, "!=") == 0) {
      //Not equals computation
      expect_next_token(pws, "M", "Invalid !=", offset);
      next_token_or_error(pws, cmds, offset);
      mpq_t& M = voc(cmds, temp_q);
      expect_next_token(pws, "X1", "Invalid !=", offset);
      next_token_or_error(pws, cmds, offset);
      mpq_t& X1 = voc(cmds, temp_q);
      expect_next_token(pws, "X2", "Invalid !=", offset);
      next_token_or_error(pws, cmds, offset);
      mpq_t& X2 = voc(cmds, temp_q2);
      expect_next_token(pws, "Y", "Invalid !=", offset);
      next_token_or_error(pws, cmds, offset);
      mpq_t& Y = voc(cmds, temp_q);
      if (mpq_equal(X1, X2)) {
        mpq_set_ui(M, 0, 1);
      ...
      next_token_or_error(pws, cmds, offset);
      mpq_t& Y = voc(cmds, temp_q);
      expect_next_token(pws, "=", ("Invalid "+std::string(tok)).c_str(), offset);
      next_token_or_error(pws, cmds, offset);
      mpq_t& X1 = voc(cmds, temp_q);
      expect_next_token(pws, tok, ("Invalid "+std::string(tok)).c_str(), offset);
      next_token_or_error(pws, cmds, offset);
      mpq_t& X2 = voc(cmds, temp_q2);
      if (strcmp(tok, "/") == 0) {
        //Exact division
      ...
      compute_split_unsignedint(pws, offset);
    } else if (strcmp(tok, "SIL") == 0) {
      compute_split_int_le(pws, offset);
    } else if (strcmp(tok, "MATRIX_VEC_MUL") == 0) {
      compute_matrix_vec_mul(pws, offset);
    } else if (strcmp(tok, "DB_GET_BITS") == 0) {
      compute_db_get_bits(pws, offset);
    } else if (strcmp(tok, "DB_PUT_BITS") == 0) {
      compute_db_put_bits(pws, offset);
    } else if (strcmp(tok, "DB_GET_SIBLING_HASH") == 0) {
      compute_db_get_sibling_hash(pws, offset);
    } else if (strcmp(tok, "GET_BLOCK_BY_HASH") == 0) {
      compute_get_block_by_hash(pws, offset);
    } else if (strcmp(tok, "PUT_BLOCK_BY_HASH") == 0) {
      compute_put_block_by_hash(pws, offset);
    } else if (strcmp(tok, "FREE_BLOCK_BY_HASH") == 0) {
      compute_free_block_by_hash(pws, offset);
    } else if (strcmp(tok, "GENERICGET") == 0) {
      compute_genericget(pws, offset);
    } else if (strcmp(tok, "PRINTF") == 0) {
      compute_printf(pws, offset);
      ...
      compute_fast_ramget(pws, offset);
    } else if (strcmp(tok, "RAMPUT_FAST") == 0) {
      compute_fast_ramput(pws, offset);
    } else if (strcmp(tok, "BENES_NETWORK") == 0) {
      //compute_benes_network(pws, offset);
    } else if (strcmp(tok, "WAKSMAN_NETWORK") == 0) {
      compute_waksman_network(pws, offset);
    } else if (strcmp(tok, "EXO_COMPUTE") == 0) {
      compute_exo_compute(pws, offset);
    } else if (strcmp(tok, "EXT_GADGET") == 0) {
      compute_ext_gadget(pws, offset);
    } else {
...
void ComputationProver::compute_from_pws_strV2(const char* pws) {
  
  FILE* pws_file = fmemopen((void *)pws, pws_length, "r");
