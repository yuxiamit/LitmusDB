...
def expand_db_ops_in_spec(spec_file, outputFile=None):
  global has_state
...
def generate_memory_consistency_in_spec(spec_file, outputFile=None):
...
def write_params(num_constraints, outFile=None):
...
def append_file_strs(fp, content):
  fp.write(content)

def append_files(fp, file_name_to_append):
  if isinstance(file_name_to_append, str):
    with open(file_name_to_append) as file_object:
      for line in file_object:
        fp.write(line);
  else:
    fp.write(file_name_to_append.getvalue())

def generate_zaatar_matrices(spec_file, shuffled_indices, qap_file_name, outFileList=None):
  ...
  if outFileList:
    fp_matrix_a = outFileList[0]
    fp_matrix_b = outFileList[1]
    fp_matrix_c = outFileList[2]
  else:
...
Expecting `[{{aIndex: aValue, ...}}, {{bIndex: bValue, ...}}, {{cIndex: cValue, ...}}]`
  ...
  if outFileList:
    write_params(num_constraints, outFileList[3])
  else:
  ...
  if outFileList:
    fp = outFileList[4]
    append_file_strs(fp, fp_matrix_a.getvalue())
    fp.write("\n");
    append_file_strs(fp, fp_matrix_b.getvalue())
    fp.write("\n");
    append_file_strs(fp, fp_matrix_c.getvalue())
    fp.write("\n");
...
def generate_computation_worksheet(spec_file, pws_loc, outputFile=None):
  global mem_timestamp
  global mem_ops_input
...
def generate_constants(consts_path):
  if isinstance(consts_path, str):
    f = open(consts_path, "r")
  else:
    f = consts_path
  code = ""
  for line in f:
