...
class CodeGenerator():
  ...
  def generate_pws(self, spec_file, defs, outputFile=None):
    ...
    (defs['num_io_vars'], defs['num_z_vars']) = zcc_parser.generate_computation_worksheet(spec_file, pws_abs_loc, outputFile) #leads to variable creation

    if (zcc_parser.verbose):
      print ("Created prover worksheet, result will appear at "+pws_loc)
  ...
  def generate_matrices(self, spec_file, defs, outputFile=None):
    ...
    if (self.framework=="GINGER"):
      ...
      if outputFile !=None:
        mA = StringIO.StringIO()
        mB = StringIO.StringIO()
        mC = StringIO.StringIO()
        mParams = StringIO.StringIO()
        mQAP = StringIO.StringIO()
        (defs['NzA'], defs['NzB'], defs['NzC'], defs['num_constraints']) = zcc_parser.generate_zaatar_matrices(spec_file, shuffledIndices, qap_file_name, [mA, mB, mC, mParams, mQAP])
        spec_file.seek(0)
        return mA, mB, mC, mParams, mQAP
      else:
  ...
  def generate_constants_file(self, spec_path, defs, constants=None):
    if constants!= None:
      defs['constants'] = zcc_parser.generate_constants(constants)
    else:
  ...
  def generate_verifier(self, spec_file, defs, skipInputGen=False):
    if (self.framework=="GINGER"):
      self.write_ginger(defs, spec_file)
    else:
      self.write_zaatar(defs, skipInputGen)

    spec_file.seek(0)

  def generate_code_from_template(self, spec_path, circuit=None, constants=None, constraints=None):

    if constants!= None:
      spec_file = StringIO.StringIO(constraints)
      zcc_parser.parse_spec_file(spec_file)

      if (zcc_parser.verbose):
        print ("Expanding database operations in " + spec_path)
      
      expanded = StringIO.StringIO()
      spec_file = zcc_parser.expand_db_ops_in_spec(spec_file, outputFile=expanded)

      mem_consistency = StringIO.StringIO()

      spec_file = zcc_parser.generate_memory_consistency_in_spec(spec_file, outputFile=mem_consistency)
    else:
    ...
    outWorkSheet = StringIO.StringIO()

    self.generate_pws(spec_file, defs, outputFile=outWorkSheet)
    print('Finished generating prover worksheet')
    #number of variables, number of basic constraints (chi) fixed from this point onwards
    outMat = StringIO.StringIO()
    mA, mB, mC, mParams, mQAP = self.generate_matrices(spec_file, defs, outputFile=outMat)

    #Write the prover
    #self.generate_prover(defs)

    #Write the constants file
    self.generate_constants_file(spec_path, defs, StringIO.StringIO(constants))
    ...
    # allow write to those verifier headers
    
    self.generate_verifier(spec_file, defs, True)

    spec_file.close()

    return outWorkSheet.getvalue(), mA.getvalue(), mB.getvalue(), mC.getvalue(), mParams.getvalue(), mQAP.getvalue()
  ...
  def write_zaatar(self, defs, skipInput=False):
    # TODO: question: why so many commented-out code here
    # the below code only writes to a input generator?
...
def shortcut(circuit, constants, constraints):
  print("In Python, size of circuit, constants, constraints are", len(circuit), len(constants), len(constraints))
  zcc_parser.printMetrics = 1
  zcc_parser.proverBugginess = 0

  zcc_parser.merkle_gen = merkle.MerkleConsGen("ggh", zcc_parser.verbose)
  zcc_parser.db_size = 1024
  zcc_parser.ram_cell_num_bits = 32
  zcc_parser.word_width = 64
  zcc_parser.address_width = 32

  os.chdir("../pequin/compiler/backend")

  gen = CodeGenerator("../../pepper", "gen/wrapped_transaction", "wrapped_transaction",	
      "ZAATAR", 1, "c")
  outWorkSheet, mA, mB, mC, mParams, mQAP = gen.generate_code_from_template("../../../dbx1000_logging/wrapped_transaction.c.ZAATAR.spec", circuit=circuit, constants=constants, constraints=constraints)
  print("Matrix Sizes in Python", len(mA), len(mB), len(mC))
  return (outWorkSheet, mA, mB, mC, mParams, mQAP)
