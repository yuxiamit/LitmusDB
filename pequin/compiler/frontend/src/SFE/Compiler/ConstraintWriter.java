...
public class ConstraintWriter {
	...
	private String circuitSource;
	private boolean useSource;

	public ConstraintWriter(String source, PrintWriter out) {
		this.circuitSource = source;
		this.useSource = true;
		this.out = out;
	}
	...
	public void toConstraints(String constraintsFile)
		...
		if(this.out==null)
			this.out = new PrintWriter(new FileWriter(constraintsFile));
		else
			System.out.println("Using initialized writer.");
		...
		// inputs, but now we convert them to normal variables
		{
			...
			if (this.useSource)
			{
				br = new BufferedReader(new StringReader(this.circuitSource));
			}
			else br = new BufferedReader(new FileReader(circuitFile));
		...
		// skip exo_compute outputs here; they're not actually circuit inputs
		{
			...
			if (this.useSource)
			{
				bufferedreader = new BufferedReader(new StringReader(this.circuitSource));
			}
			else bufferedreader = new BufferedReader(new FileReader(circuitFile));
		...
		// third pass - output variables
		{
			...
			if (this.useSource)
			{
				br2 = new BufferedReader(new StringReader(this.circuitSource));
			}
			else br2 = new BufferedReader(new FileReader(circuitFile));
			System.out.println("2.5 Pass");
