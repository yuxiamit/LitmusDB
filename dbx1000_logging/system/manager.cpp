...
void Manager::init() {
	...
	*_last_epoch_update_time = get_sys_clock();
	...
	lastPSN = (uint64_t**) _mm_malloc(sizeof(uint64_t*) * g_num_logger, ALIGN_SIZE);
	for(uint32_t i=0; i<g_num_logger; i++)
	{
		lastPSN[i] = (uint64_t*) _mm_malloc(sizeof(uint64_t), ALIGN_SIZE);
		lastPSN[i][0] = 0;
	}
	#endif
	#if CC_ALG == DETRESERVE
	detreserve_table = (uint32_t *) _mm_malloc(sizeof(uint32_t) * (lock_table_mask+1), ALIGN_SIZE);
	memset(detreserve_table, -1, sizeof(uint32_t) * (lock_table_mask+1));
	#endif
