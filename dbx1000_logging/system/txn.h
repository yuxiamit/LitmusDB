...
public:
	...
	bool process_phase2(uint32_t priority) {
		// check priorities
		for(uint32_t i=0; i<row_cnt; i++)
			{
				uint32_t id = accesses[i]->hashed_key;
				if (detreserve_table[id] < priority)
				{
					cleanAfterPhase2();
		...
		{
			Access * curAccess = accesses[write_set[i]];
			//memcpy(curAccess->orig_row->data, curAccess->data, MAX_TUPLE_SIZE);
		}*/
        ...
        if(wr_cnt > 0)
		{
				...
				_cur_tid = log_manager->serialLogTxn(
                    _log_entry,
                    _log_entry_size
                );
	...
	inline bool txnEnd(){
		return wr_cnt>0;
		// skip the following code because each txn_man corresponds to a single txn in the current batch
		bool ret = true;
		if(wr_cnt==0)
			ret = false;
		_log_entry_size = 0;
