...
//private: 
	...
	volatile uint64_t  * _filled_lsn[64]; //[16];
#if SOLVE_LIVELOCK
	volatile uint64_t  * _allocate_lsn[64];
#endif
	uint64_t padding2[8];
...
public:
	...
	uint64_t	get_next_log_entry_non_atom(char * &entry, bool copybuffer=false); //, uint32_t &size);
