...
/*  According to Silo's paper, no need to validate writes
...
/*	
#elif LOG_ALGORITHM == LOG_SERIAL
	// we still need to capture the WAR relationship in order for command logging to work.
	for (uint32_t i = 0; i < row_cnt - tmp_wr_cnt; i ++) {
		Access * access = accesses[ read_set[i] ];
		row_t *row = access->orig_row;
		if (row->manager->get_tid() != access->tid) { // pre-abort again
		...
		// fetch LV to update txn_lv
		uint64_t val = row->manager->_tid_word;
		// catch WAR
		while(val < _cur_tid && !ATOM_CAS(row->manager->_tid_word, val, _cur_tid))
			PAUSE;
	} 
*/
#endif
